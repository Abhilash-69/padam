...
app.get("/api/m/:movie_name",async (req,res)=>{
    ...
    let resp = await db.query(`select * from crew where c_name='${movie_name}'`);
    if(resp.rowCount===0){
        console.log("scrap")
        const obj= await scrap_actor_info(movie_name);
        const s = await db.query("insert into crew(c_name,birthday,birthplace,about) values($1,$2,$3,$4) RETURNING *",[movie_name,obj.actor_bday,obj.actor_bplace,obj.actor_bio]);        
        res.json(s.rows[0])
    }
    else{
        res.json(resp.rows[0])
